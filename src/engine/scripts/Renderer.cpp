...
Renderer::Renderer(std::weak_ptr<GameObject> _parent) : Component(_parent) {
...
void Renderer::SetMesh(std::weak_ptr<Mesh> _mesh) {
	m_Mesh = _mesh.lock();
}
...
void Renderer::SetMaterial(std::weak_ptr<Material> _material) {
...
void Renderer::SetTransform(std::weak_ptr<Transform> _transform) {
