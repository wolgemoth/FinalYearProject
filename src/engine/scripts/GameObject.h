...
public:
	...
	[[nodiscard]] static std::shared_ptr<GameObject> Create(std::weak_ptr<Scene> _scene, std::string _name = "");
	...
	/// <returns>std::weak_ptr<T> Referencing the component if successful. std::weak_ptr<T> referencing a nullptr if unsuccessful.</returns>
	template <typename T>
	std::weak_ptr<T> GetComponent(size_t _index = 0) {
	...
	/// <returns>std::weak_ptr<T> referencing the created type.</returns>
	template <typename T>
	std::weak_ptr<T> AddComponent() {
		...
		auto result = std::shared_ptr<T>(new T(weak_from_this()));
		...
		else if (m_Components.Add(category, entries) == false) {
			result.reset();
	...
	void RemoveComponent(size_t _index = 0) {
		...
		if (m_Components.Get(typeid(T), entries)) {
			...
			if (_index < entries.size()) {
				...
				entries.erase(entries.begin() + _index);
			}
		}
	}
};
