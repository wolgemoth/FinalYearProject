...
namespace LouiEriksson::Engine::Spatial::Serialisation {
	...
	ElevationDeserialiser::OTDJSON::Root::Root(json _otd_json) :
		results()
	{
		const auto jResults = _otd_json["results"];
		
		for (const auto& item : jResults) {
			results.emplace_back(item);
		}
	}
	
	ElevationDeserialiser::OTDJSON::Root::Root(const std::vector<Result>& _results) :
		results(_results) {}
	...
	ElevationDeserialiser::OEJSON::Root::Root(json _oe_json) :
		results()
	{
		const auto jResults = _oe_json["results"];
