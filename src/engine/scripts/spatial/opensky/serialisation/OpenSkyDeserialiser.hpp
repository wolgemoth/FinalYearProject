				...
				struct StateVector final {
						...
						vertical_rate(), sensors(), geo_altitude(), squawk(), spi(), position_source(), category()
                    {
						try {
							...
							try {          icao24 = _os_json[ 0]; } catch (const json::exception& e) {         icao24 = std::nullopt; }
							try {        callsign = _os_json[ 1]; } catch (const json::exception& e) {       callsign = std::nullopt; }
							try {  origin_country = _os_json[ 2]; } catch (const json::exception& e) { origin_country = std::nullopt; }
							try {   time_position = _os_json[ 3]; } catch (const json::exception& e) {  time_position = std::nullopt; }
							try {    last_contact = _os_json[ 4]; } catch (const json::exception& e) {   last_contact = std::nullopt; }
							try {       longitude = _os_json[ 5]; } catch (const json::exception& e) {      longitude = std::nullopt; }
							try {        latitude = _os_json[ 6]; } catch (const json::exception& e) {       latitude = std::nullopt; }
							try {   baro_altitude = _os_json[ 7]; } catch (const json::exception& e) {  baro_altitude = std::nullopt; }
							try {       on_ground = _os_json[ 8]; } catch (const json::exception& e) {      on_ground = std::nullopt; }
							try {        velocity = _os_json[ 9]; } catch (const json::exception& e) {       velocity = std::nullopt; }
							try {      true_track = _os_json[10]; } catch (const json::exception& e) {     true_track = std::nullopt; }
							try {   vertical_rate = _os_json[11]; } catch (const json::exception& e) {  vertical_rate = std::nullopt; }
							
							try {
								
								auto jSensors = _os_json[12];
							...
							try {    geo_altitude = _os_json[13]; } catch (const json::exception& e) {    geo_altitude = std::nullopt; }
							try {          squawk = _os_json[14]; } catch (const json::exception& e) {          squawk = std::nullopt; }
							try {             spi = _os_json[15]; } catch (const json::exception& e) {             spi = std::nullopt; }
							try { position_source = _os_json[16]; } catch (const json::exception& e) { position_source = std::nullopt; }
							try {        category = _os_json[17]; } catch (const json::exception& e) {        category = std::nullopt; }
