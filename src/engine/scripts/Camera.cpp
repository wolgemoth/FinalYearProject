...
namespace LouiEriksson {
	...
	Camera::Camera(std::weak_ptr<GameObject> _parent) : Component(std::move(_parent)) {
	...
	void Camera::Draw(const std::weak_ptr<Renderer>& _renderer) {
	...
	const glm::mat4& Camera::Projection() {
		...
		// Recalculate the perspective matrix if dirtied.
		if (m_IsDirty) {
			m_IsDirty = false;
