...
namespace LouiEriksson {
	...
	std::shared_ptr<Shader> Shader::Create(const std::vector<Shader::SubShader>& _subShaders) {
		return {new Shader(_subShaders), [](Shader* _ptr) { delete _ptr; }};
	}
	...
	GLint Shader::GetUniform(const char* _name) const {
		return glGetUniformLocation(this->ID(), _name);
	}
	
	void Shader::BindAttribute(GLuint _pos, const char* _name) const {
